---
name: swarm-worker-glm
description: "[GLM-5.2 variant] Implementation worker for the swarm skill. Receives one atomic task from the orchestrator, makes the smallest correct change, runs verification, reports evidence. Has full toolset for editing, running tests, calling skills/MCP. Use when the swarm skill's start-work pattern dispatches implementation tasks to parallel workers."
tools: ["*"]
...
<!-- Explicit GLM-5.2 variant of swarm-worker. Use when orchestrator wants to guarantee GLM-5.2 dispatch (e.g., primary fallback in a chain). -->
